...
public class PageUtil<T> {
	...
	/**
	 * 必要的数据项 ： 计算出 其他的数据项： 数据总行数 rowcount ； 一页显示的数据行数 pagesize 导航栏 导航的个数 navnum
	 * ；当前页导航 currnav
	 * 
	 */
	public PageUtil(int rowcount, int pagesize, int currnav, int navnum) {
		...
		/**
		 * 6 总页数 navcount, ceil函数 向上取整
		 */
		this.navcount = (int) Math.ceil(rowcount * 1.0 / pagesize);
		/**
		 * 7 起始导航 6 6-5 this.begin =1 当前页 5 5-5 =0； 4 -5 =-1
		 */
		...
		/**
		 * 9 上一页 如果是第一页 没有上一页 1 页 0 1 比较 取最大值
		 */
		this.prev = Math.max(this.currnav - 1, 1);
		/**
		 * 10 下一页 如果是最后一页 没有下一页 76 + 1 = 77 77没有 76 和77 比较 取最小
		 */
		...
		/**
		 * 特殊情况处理 一共 760 行数据 当前点击的导航页 65 int rowcount = 760 ; //数据总行数 int
		 * pagesize = 10 ; //每页显示的行数 int currnav = 75; //当前点击的导航 页 当前页 结果：错误
		 * this.begin = 70; this.end = 76 应该是： end -begin = 9
		 */
